...
public class EasyTouch : MonoBehaviour {
	...
	public delegate void OverUIElementHandler( Gesture gesture, EventName eventName);
    ...
    //cly
    private bool useEventWhenOverGui = false;//碰到ui也要执行event
	#endregion
	...
	private void CreateGesture(int touchIndex,EventName message,Finger finger, SwipeDirection swipe, float swipeLength, Vector2 swipeVector){
		...
		if ( firingEvent){
			...
			if (message == EventName.On_TouchUp){
				...
				RaiseEvent(EventName.On_OverUIElement, gesture, message);
			}
		}
	...
	public void RaiseEvent(EventName evnt, Gesture gesture, EventName oerGuiEventName = EventName.None){
		...
		switch(evnt){
			...
			case EventName.On_OverUIElement:
				if (On_OverUIElement!=null){
					On_OverUIElement(gesture, oerGuiEventName);
				}
				break;
    ...
    public static void SetUseEventWhenOverGui(bool value){
		if (EasyTouch.instance != null){
			EasyTouch.instance.useEventWhenOverGui = value;
		}
	}
    public static bool GetUseEventWhenOverGui(){
		if (EasyTouch.instance != null){
			return EasyTouch.instance.useEventWhenOverGui;
		}
        return false;
	}
	#endregion
