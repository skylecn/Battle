...
public class vp_Timer : MonoBehaviour
{
	...
	static float _pausedTime = -1f;
	
	void OnApplicationPause(bool pauseStatus)
	{
		_pausedTime = pauseStatus ? Time.time : -1f;
#if UNITY_EDITOR
		if (_pausedTime < 0)
		{
			EditorRefresh();
	...
	private void LateUpdate()
	{
		...
		while ((vp_Timer.m_Active.Count > 0) && m_EventBatch < MaxEventsPerFrame)
		{
			...
			{
				if (vp_Timer.m_Active[m_EventIterator].Id == 0 || !vp_Timer.m_Active[m_EventIterator].Paused)
				{
					vp_Timer.m_Active[m_EventIterator].Execute();
				}
				else
				{
					vp_Timer.m_Active[m_EventIterator].DueTime += deltaTime;
				}
			}
			else
			{
				...
				// log lifetime
					vp_Timer.m_Active[m_EventIterator].LifeTime += deltaTime;
			}
	...
	private static void Schedule(float time, Callback func, ArgCallback argFunc, object args, Handle timerHandle, int iterations, float interval)
	{
		...
		float curTime = _pausedTime > 0 ? _pausedTime : Time.time;
		m_NewEvent.StartTime = curTime;
		m_NewEvent.DueTime = curTime + time;
	...
	{
		if (_pausedTime < 0&& m_MainObject!=null)
		{
	...
	private class Event
	{
		...
		public void Execute()
		{
			...
			if (Function != null)
			{
				try
				{
#if UNITY_EDITOR && IL_PERF
					string name = null;
					
					if (Attribute.IsDefined(Function.Method.DeclaringType, typeof(CompilerGeneratedAttribute)))
					{
						FieldInfo fi = Function.Target.GetType().GetField("act",BindingFlags.Public|BindingFlags.NonPublic|BindingFlags.Instance|BindingFlags.Static);
						var act=fi.GetValue(Function.Target) as Action;
						
						name = "vp_timer_" + act.Target;
					}
					else
					{
						name = "vp_timer_" + Function.Target + "_" + Function.Method.Name;
					}
					long time1 = ILRuntimePerf.GetNanoseconds();
					UnityEngine.Profiling.Profiler.BeginSample(name);
					Function();
					UnityEngine.Profiling.Profiler.EndSample();
					long time2 = ILRuntimePerf.GetNanoseconds();
					ILRuntimePerf.AddTimerPerf(name, (time2 - time1));
#else
					Function();
#endif
				...
				{
					UnityEngine.Debug.LogError("vp_Timer exception:" + e.Message + " " + e.StackTrace);
				}
			}
...
#if UNITY_EDITOR && IL_PERF
					string name = null;
					if (Attribute.IsDefined(ArgFunction.Method.DeclaringType, typeof(CompilerGeneratedAttribute)))
					{
						FieldInfo fi = ArgFunction.Target.GetType().GetField("act", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Static);
						var act = fi.GetValue(ArgFunction.Target) as Action<object>;

						name = "vp_timer_" + act.Target;
					}
					else
					{
						name = "vp_timer_" + ArgFunction.Target + "_" + ArgFunction.Method.Name;
					}
					long time1 = ILRuntimePerf.GetNanoseconds();
					UnityEngine.Profiling.Profiler.BeginSample(name);
	...
	public class Handle
	{
		...
		public int Id
		{
			...
			set
			{
				...
				if (m_Id == 0)
				{
                    //�����ڻص�����cancel�Լ�������DueTime�޷��쵽�����Ըĳ�����m_Event.Id = 0,CancelAll ��������������õ�Id = 0
